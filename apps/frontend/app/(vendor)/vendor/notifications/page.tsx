    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[#A89F91]">Stay up to date on new RFQs, order statuses, and payments.</p>
        </div>
        {hasUnread && (
          <button
            type="button"
      ...
      {!notificationsQuery.isLoading && !notificationsQuery.isError && notifications.length === 0 && (
        <div className="bg-[#1A1714] border border-dashed border-[#2A2520] rounded-2xl p-16 text-center">
          ...
          <p className="mt-1 text-sm text-[#A89F91]">You&apos;ll be notified when new RFQs match your profile, orders are confirmed, and more.</p>
        </div>
      )}
